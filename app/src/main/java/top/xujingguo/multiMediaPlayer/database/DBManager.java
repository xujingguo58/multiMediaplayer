package top.xujingguo.multiMediaPlayer.database;
...
public class DBManager {
    ...
    public MusicInfo getSingleMusicFromMusicTable(int id) {
        ...
        db.beginTransaction();
        try {
            cursor = db.query(DatabaseHelper.MUSIC_TABLE, null, DatabaseHelper.ID_COLUMN + " = ?", new String[]{"" + id}, null, null, null);
            musicInfoList = cursorToMusicList(cursor);
    ...
    public List<PlayListInfo> getMyPlayList() {
        ...
        while (cursor.moveToNext()) {
            ...
            int id = Integer.valueOf(cursor.getString(cursor.getColumnIndex(DatabaseHelper.ID_COLUMN)));
            playListInfo.setId(id);
            playListInfo.setName(cursor.getString(cursor.getColumnIndex(DatabaseHelper.NAME_COLUMN)));
            cursorCount = db.query(DatabaseHelper.PLAY_LISY_MUSIC_TABLE,null, DatabaseHelper.ID_COLUMN + " = ?", new String[]{"" + id}, null,null,null);
    ...
    public ArrayList<Integer> getMusicIdListByPlaylist(int playlistId){
        ...
        try{
            String sql = "select * from "+DatabaseHelper.PLAY_LISY_MUSIC_TABLE+" where "+ DatabaseHelper.ID_COLUMN+" = ? ";
    ...
    public List<MusicInfo> getMusicListByPlaylist(int playlistId){
        ...
        db.beginTransaction();
        try{
            String sql = "select * from "+DatabaseHelper.PLAY_LISY_MUSIC_TABLE+" where "+ DatabaseHelper.ID_COLUMN+" = ? ORDER BY "+ DatabaseHelper.ID_COLUMN;
            cursor = db.rawQuery(sql,new String[]{""+playlistId});
            while (cursor.moveToNext()){
                MusicInfo musicInfo = new MusicInfo();
                id =  cursor.getInt(cursor.getColumnIndex(DatabaseHelper.MUSIC_ID_COLUMN));
    ...
    public void insertMusicInfoToMusicTable(MusicInfo musicInfo) {
        ...
        try {
            ...
            values.put(DatabaseHelper.ID_COLUMN, id);
//			values.put("mylove",0);
    ...
    public boolean isExistPlaylist(int playlistId,int musicId){
        ...
        Cursor cursor = db.query(DatabaseHelper.PLAY_LISY_MUSIC_TABLE,null, DatabaseHelper.ID_COLUMN + " = ? and "+ DatabaseHelper.MUSIC_ID_COLUMN + " = ? ",
    ...
    public void deleteMusic(int id) {
        db.execSQL("PRAGMA foreign_keys=ON");
        db.delete(DatabaseHelper.MUSIC_TABLE, DatabaseHelper.ID_COLUMN + " = ? ", new String[]{"" + id});
        db.delete(DatabaseHelper.LAST_PLAY_TABLE, DatabaseHelper.ID_COLUMN + " = ? ", new String[]{"" + id});
    }

    public void deletePlaylist(int id) {
        db.delete(DatabaseHelper.PLAY_LIST_TABLE, DatabaseHelper.ID_COLUMN + " = ? ", new String[]{"" + id});
    }
    ...
    public void removeMusic(int id, int witchActivity) {
        ...
        switch (witchActivity) {
            ...
            case Constant.ACTIVITY_MYLOVE:
                ...
                db.update(DatabaseHelper.MUSIC_TABLE, null, DatabaseHelper.ID_COLUMN + " = ? ", new String[]{"" + id});
                break;
        }
    }
    ...
    public int removeMusicFromPlaylist(int musicId, int playlistId) {
        ...
        int ret = 0;
        try {
            ret = db.delete(DatabaseHelper.PLAY_LISY_MUSIC_TABLE, DatabaseHelper.ID_COLUMN + " = ? and " + DatabaseHelper.MUSIC_ID_COLUMN
        ...
        try {
            ...
            if (lastList.size() < 20) {
                for (int i = 0; i < lastList.size(); i++) {
                    values.put(DatabaseHelper.ID_COLUMN, lastList.get(i));
    ...
    public void setMyLove(int id) {
        ...
        db.update(DatabaseHelper.MUSIC_TABLE, values, DatabaseHelper.ID_COLUMN + " = ? ", new String[]{"" + id});
    }
